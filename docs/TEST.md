...
> 對應版本:v2.0.1(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| Isaac HIL（人工啟動） | Actions → `Isaac HIL Acceptance`，或依 `docs/ISAAC_HIL_ACCEPTANCE.md` 執行 | 一般 CI 絕不動車；精確確認後在 self-hosted runner 驗 route、cancel、hard stop、可選 Twin verdict。2026-07-19 已有 clean `fb56456…b1e` 本機 artifact：兩條 route、cancel/stop 與 scan gate 通過，Twin 同 domain 明記 skip；這不等於已產生 GitHub workflow artifact |
...
- **FullScan HIL（2026-07-19）**：RTX Helper `Publish Full Scan=True`；Nav2 使用由完整點雲裁出的前向 180° `/scan`。正式 sample 10/10、0 blank、finite-bin 53.7569%；`map_left_down` 66.985 s、Dock 46.754 s，皆 0 recoveries；cancel propagated、停止漂移 0.0000 m。clean `fb56456…b1e`，overall `pass_with_skips`（Twin 同 domain 0）
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 2.0.1`(版本來自 package metadata,隨 release 走) |
...
| 狀態 | 指令 | 測法 | 期望輸出 |
|---|---|---|---|
| ✅ | `/route 從應科大樓到機械系館` | 建點 + (Nav2 或 route_adapter=odom) | **兩端已知 → 先去 A 再去 B**(兩段導航);只認得目的地 → 從當前位置去。即時剩餘距離、Esc 真取消。2026-07-19 正式 HIL：cancel propagated、零速度 halt，停止後漂移 0.0000 m ✅（模擬） |
