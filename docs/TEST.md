...
> 對應版本:v0.23.2(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 單元測試(全) | `env -u PYTHONPATH uv run pytest` | 全綠(v0.23.2 基準 `376 passed`,約 25s,無 ROS 環境也全過);含安全鏈故障注入(bridge 啟動失敗/watchdog 武裝失敗/twin 預演中斷/halt 失敗誠實回報)與架構鐵律測試 |
