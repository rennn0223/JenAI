...
> 對應版本:v1.1.1(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 自動化測試(全) | `env -u PYTHONPATH uv run pytest` | 全綠(v1.1.1 現況 505 項,無 ROS 環境也全過);含安全鏈故障注入、輸入邊界、指令 FIFO/批准暫停/stop 清隊列與架構鐵律測試 |
...
## 本機實測現況快照(v1.1.1 實測,DGX Spark 工作機,Isaac Sim 倉庫場景)
...
- **B4 preflight**:目前四角一圈 3/4；左下因 Twin Gate G5 Nav2 aborted 被 refer，
  執行側 goal 未送出。這是補充測試，不取代原約 20 小時的 407/408 結果。
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 1.1.1`(版本來自 package metadata,隨 release 走) |
