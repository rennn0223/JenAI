...
> 對應版本:v0.18.0(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 單元測試(全) | `env -u PYTHONPATH uv run pytest` | 全綠(v0.18.0 基準 `352 passed`,約 24s,無 ROS 環境也全過);含安全鏈故障注入(bridge 啟動失敗/watchdog 武裝失敗/twin 預演中斷/halt 失敗誠實回報)與架構鐵律測試 |
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 0.18.0`(版本來自 package metadata,隨 release 走) |
...
| 🔶 | 反應式避障(`[avoidance] enabled=true`,route_adapter=odom) | 需 depth topic + 障礙 | odom 直驅時 depth→follow-the-gap:偵測太近自動繞開、清空後回到原線;實測 Isaac 障礙前 1.1m → 觸發避障、側向偏 1.2m 未撞上。純律 `follow_the_gap` 7 個單元測試 |
