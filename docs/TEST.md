...
> 對應版本:v1.1.2(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 自動化測試(全) | `env -u PYTHONPATH uv run pytest` | 全綠(v1.1.2 現況 505 項,無 ROS 環境也全過);含安全鏈故障注入、輸入邊界、指令 FIFO/批准暫停/stop 清隊列與架構鐵律測試 |
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 1.1.2`(版本來自 package metadata,隨 release 走) |
