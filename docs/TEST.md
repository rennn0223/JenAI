...
> 對應版本:v0.36.0(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
1. ~~route 解析器對 destination-only 句式(`去X`、`Go to X`)失敗~~ → **v0.36.0 已修**
   (goal-only regex 快路 + LLM fallback 接受空 start;起點一律取機器人當前位置)。
2. ~~agent 的 `ros_state` 讀不到位姿~~ → **v0.36.0 已修**(新增 `pose` 欄位,latched QoS
...
3. **【最高優先】`/run` 批准後工具不執行**:audit 證據——resume 轉 running 到 blocked
   ...
   「模型迴圈」。先前 v0.36.0 notes 的「35b 模型迴圈」敘事**是錯的**,真兇在 approval
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 0.36.0`(版本來自 package metadata,隨 release 走) |
...
| ✅ | `/run 帶我到大廳` | Supervisor handoff 給專職 agent 執行;side-effect 工具一律過批准卡。**前置:Nav2+地點**(無後端時誠實失敗)。實測(Isaac Carter,qwen3.6:35b,v0.36.0):NL「帶我去dock」全鏈 E2E ✅——解析→批准→執行→Nav2 到點 0.06m;模型第一次給壞 JSON 被誠實拒絕後自行重試成功 |
| ✅ | `/why` | 解釋 agent 當前決策原因 |
...
| ✅ | `/loc add here 測試點` | 需 /amcl_pose 或 /odom | 抓當下位置存檔。實測(Isaac Carter,v0.36.0):靜止讀 /amcl_pose 四點入檔 ✅(QoS 修復後) |
