...
|---|---|---|---|---|---|
| ENG-1 | 工程師 | 無 ROS 的單元／整合測試、lint、三版本 CI、wheel 冒煙全綠 | 532 tests 與完整 lint 本機全綠；Python 3.12–3.14 CI；安全鏈 coverage gate；v1.1.4 wheel | PASS | 每次 PR 持續維持 |
| ENG-2 | 工程師 | ROS／Isaac 關鍵路徑可自動回歸，不只人工 TUI 實測 | 已有明確批准、fail-closed 的 `isaac-hil.yml` 與 JSON schema；2026-07-18 真 graph 唯讀 preflight 保存 required-nav 全 pass 與同域 `twin_isolation=fail`，未誤稱 Twin 通過；尚缺 live workflow artifact | PARTIAL | Reset 合法起點後跑 route、取消、stop；隔離 Twin 場景另跑 verdict，保存成功與失敗 artifact |
| ENG-3 | 工程師 | 核心模組職責可維護 | 抽出 `tui/catalog.py`、`LocationCommandsMixin` 與 stdlib-only `bridge/_protocol.py`；app 1,789→1,562、robot_commands 1,101→812、ros_bridge 935→875 行；84 項 TUI/location 與 30 項 bridge 回歸通過 | PARTIAL | app 批准／執行與 bridge node 仍偏大，繼續拆分並維持完整行為測試 |
