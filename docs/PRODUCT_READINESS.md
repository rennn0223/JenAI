...
|---|---|---|---|---|---|
| ENG-1 | 工程師 | 無 ROS 的單元／整合測試、lint、三版本 CI、wheel 冒煙全綠 | strict pytest 597/597、0 warnings、安全鏈 coverage 93%；PR #106 與 main@`79a295a` 的 Python 3.12／3.13／3.14 CI、build 及 Release gate 全綠，run URL 如上 | PASS | 後續每個 PR／main revision 持續維持三版本 CI 與 safety-chain gate |
| ENG-2 | 工程師 | ROS／Isaac 關鍵路徑可自動回歸，不只人工 TUI 實測 | HIL runner 有雙重 opt-in、NavigationGateway、scan-quality gate、合法起點重檢與 cancel／halt；2026-07-19 clean `fb56456…b1e` artifact 通過兩條 route、cancel/stop，Twin 同 domain 明記 skip | PASS | 每個候選 release 維持同 protocol 回歸；separated-domain Twin verdict 另由安全／部署 gate 驗收 |
...
| RES-1 | 教授 | 研究問題、方法、證據與限制一致 | `EVIDENCE_LEDGER` 明列 E2 A／B derived、C observed，B4 102 reports／407-of-408；HIL-FS-20260719 另有 clean execution revision 與必要 preflight。較早 E1、E2-C、E3、E4、B4 仍缺 execution revision／歷史 doctor，B4 也無 incident 欄或獨立觀察者 | PARTIAL | 重跑或找回歷史 metadata；補獨立事件觀察。新結果只能追加且保留失敗，模擬 HIL 不外推實體 |
...
| SALES-1 | 業務 | 三分鐘內可穩定展示核心價值 | HIL-FS-20260719 已在 clean commit 完成 `map_left_down`、Dock 與 cancel/stop；這是 2 條成功 route，不是 Hero 10-run 成功率 | PARTIAL | 同 commit／模型／場景連跑 10 次，≥9 次完整成功並保留所有失敗 |
...
本輪先以本機 frozen snapshot 驗 Ruff、597/597、0 warnings、安全鏈 coverage 93% 與
...
constraints、逐版 SBOM 或 provenance，也不能取代不同 revision 的 HIL 或使用者研究。HIL-FS-20260719 另屬 clean `fb56456…b1e` 的本機證據，不能回填到 v2.0.1 release assets。
...
|---|---|---|---|
| 工程師 | `SATISFIED_WITH_EXTERNAL_GATES` | 原有 CI／release 證據，加上 clean `fb56456…b1e` 的 FullScan HIL：scan gate、兩條 route、cancel/stop 全通過 | separated-domain Twin verdict；每版 self-hosted workflow artifact |
...
| 教授 | `SATISFIED_WITH_EXTERNAL_GATES` | `EVIDENCE_LEDGER`、`USABILITY_STUDY`、`SAFETY_CASE` 與 HIL-FS-20260719；derived／observed／sim-only 限制已對齊 | prospective 使用者研究、獨立事件觀察與非 Ackermann 實體 PoC；歷史 metadata 缺口仍在 |
| 業務 | `SATISFIED_WITH_EXTERNAL_GATES` | `PRODUCT_BRIEF`、`DEMO_SCRIPT` 與 HIL-FS-20260719；核心 route／Dock／cancel 已有正式模擬樣本 | Isaac Hero 10-run 尚未執行；效率結果與可引用案例 |
...
目前外部事實更新為：clean `fb56456…b1e` 已在合法 Dock 起點完成兩條 Isaac/Nav2 route
與 cancel/stop，scan-quality gate 通過；Twin 因 target／configured twin 同 domain 0 明記
`skip`。這仍不是 10-run、實體安全或跨載具證據。UI 樣本已獲接受並封版；v2.0.1
