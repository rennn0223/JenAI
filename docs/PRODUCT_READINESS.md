...
JenAI v2.0.2 候選是一個**受監督、具執行邊界的 ROS2 高階決策與工作流代理**。它使用自然語言
...
|---|---|---|---|---|---|
| ENG-1 | 工程師 | 無 ROS 的單元／整合測試、lint、三版本 CI、wheel 冒煙全綠 | v2.0.2 候選本機 652/652、Ruff 全綠；PR #106 與 main@`79a295a` 的 597/597、Python 3.12／3.13／3.14 CI、build 及 Release gate 是上一版獨立證據 | PARTIAL | 候選 PR 的三版本 CI、build、wheel smoke 全綠後轉 PASS |
...
| PM-2 | PM | v2 與 post-v2 承諾分開 | v2.0.2 候選仍是受監督工作流代理與產品化安全基線；M6 常駐決策迴圈移至 post-v2（候選 v3），目前未實作 | PASS | README、論文、demo 不得把 M6 當現有能力 |
...
constraints、逐版 SBOM 或 provenance，也不能取代不同 revision 的 HIL 或使用者研究。v2.0.2 候選另有本機 652/652、clean `d942130…855` HIL-FS2 與 clean `cc6d217…f6e` Hero10；它們不能回填到 v2.0.1 release assets，也尚未取代候選 PR CI／wheel gate。
...
|---|---|---|---|
| 工程師 | `SATISFIED_WITH_EXTERNAL_GATES` | 原有 CI／release 證據，加上 clean `d942130…855` 的完整 FullScan gate、兩條 route、cancel acknowledgement／software halt，以及 clean Hero 10/10 legs | 候選 PR CI／wheel；separated-domain Twin verdict；每版 self-hosted workflow artifact |
