...
constraints、逐版 SBOM 或 provenance，也不能取代尚未完成的 HIL／使用者研究。
...
|---|---|---|---|
| 工程師 | `SATISFIED_WITH_EXTERNAL_GATES` | `tests/`、`.github/workflows/{ci,release,security}.yml`、`docs/TEST.md`、`docs/SAFETY_CASE.md`、`docs/ISAAC_HIL_ACCEPTANCE.md`；597/597、93%、PR／main 三版本 CI、Supply Chain、Release assets 與 published-wheel lifecycle 均有固定 URL／revision | 合法起點的 Isaac route／cancel／stop artifact |
...
已獲使用者接受並封版；v2.0.1 已在 private repository 發布且協作者安裝路徑已驗證；既有
本機資料尚未獲准 harden；repository 仍 private 且 main／tag unprotected。受試者、
第二維護者、未授權買家交付通道、獨立觀察者與另一載具的 artifact 仍未取得。
