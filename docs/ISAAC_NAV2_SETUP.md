...
> 四階段總覽與後續的孿生側/Gate 設定見 [TWIN_SETUP](operations/TWIN_SETUP.md) §0。

---

## 0. 前置確認(一次性)

1. **先 source ROS 2 Jazzy 與 Isaac ROS workspace，再開 Isaac Sim**
   (bridge 才會使用本專案建置的 Jazzy 套件，而非其他 ROS 發行版或內建庫):
   ```bash
   source /opt/ros/jazzy/setup.bash
   source /home/nvidia/IsaacSim-ros_workspaces/jazzy_ws/install/setup.bash
   ./isaac-sim.sh   # 或你的啟動方式
   ```
   本專案使用 `jazzy_ws`；不要 source 舊的 `humble_ws`。
...
再照 [TWIN_SETUP](operations/TWIN_SETUP.md) §5 走(doctor → 建點 → `/route`)即可開始
...
3. **驗證我的避障演算法 vs Nav2**(你提的模擬驗證):同一組起終點 + 障礙,
   ...
   也直接餵 [ROADMAP](product/ROADMAP.md) 軌道 2/3。
...
bins、57.0442% valid finite。請以 [Isaac HIL preflight](validation/ISAAC_HIL_ACCEPTANCE.md) 驗證，
