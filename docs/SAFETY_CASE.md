...
> 標 ⬜ **TODO(客戶)** 的欄位需要場域資料才能完稿(對應 V1_GATE B4/B5/B6)。
>
...
| # | 危害 | 成因 | 防護 | 驗證證據 | 殘餘風險 |
|---|---|---|---|---|---|
| H1 | 撞到人/障礙物 | 導航中環境變化;規劃錯誤 | **R**:避障是 Nav2/controller 的事(毫秒級,LLM 不在迴路 —— 架構測試強制);**G**:goal 先在孿生預演,G1 碰撞即 block | test_architecture(LLM 不進反射層)、test_twin_gate(碰撞 block);⬜ TODO(客戶):實車避障實測紀錄(B4) | 感測器盲區、twin 與實景偏差 —— 靠限速與人員知情減輕 |
| H2 | 進入危險區域(樓梯口、充電樁後方) | 目標點合法但路徑/終點不當 | **G**:forbidden zones,G3 進禁區即 block(軌跡採樣);**H**:目標點由人批准 | test_twin_gate(禁區 block、終點在禁區不預演直接 block);⬜ TODO(客戶):場域禁區座標清單(B5) | 禁區清單不完整;gate 關閉時只剩 H |
...
| H6 | 緊急停止失效 | 停止路徑本身出 bug;停止被排隊 | **R+設計**:`/stop` 免批准、Esc 取消不了停止本身、halt publisher 預建、慢操作走 worker thread 不擋 halt、`nav_cancel` 取消所有 goal(跨程序);WebUI STOP 免 token | 安全鏈覆蓋 92% + CI 倒退閘;halt 失敗誠實回報(test_daemon);TEST.md `/stop` 實測 | 多重防護後仍是軟體 —— 硬體 estop 為最後防線 |
...
⬜ B4 實車里程數據(≥20h/50 任務事件表)⬜ B5 禁區清單 + gate 消融數據
⬜ B6 onboarding 實測中的安全環節確認 ⬜ 場域安全速度上限與硬體 estop 配置
