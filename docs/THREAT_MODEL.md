...
| WebUI(`JenAI web`) | 預設 127.0.0.1;`--host` 可進 LAN | **token 認證**(每次啟動自動生成,Bearer/cookie/`?token=`;`--token` 可固定)、批准動作 server 端一次性 confirm_id(瀏覽器拿不到也改不了原始 action) | **`/api/stop` 免認證** —— 停車永遠安全,手機 cookie 掉了也要停得下來;攻擊者最多能把車停下(可用性損失,非安全損失) |
...
- **瀏覽器端竄改批准內容** → 一次性 confirm_id,action 存 server 端
