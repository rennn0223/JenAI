...
- 已有證據：Isaac Sim/Nav2 高階任務、固定場景 Twin Gate、模擬巡航、daemon soak，
  ...
  不完全相同；正式數字、derived/observed 性質與缺失 metadata 以 [EVIDENCE_LEDGER](../validation/EVIDENCE_LEDGER.md) 為準。
- 2026-07-19 的 clean `d942130…855` HIL 通過現行完整 FullScan metadata gate、兩條 route、Nav2 cancel acknowledgement 與 software halt；clean `cc6d217…f6e` 另保留 0-goal preflight failure，恢復後固定序列 10/10 route legs succeeded。Twin 同 domain 0 仍為 `skip`，所以不構成通訊隔離證據；10 legs 也不是 10 次自然語言或完整 demo。
...
詳細三分鐘與 15 分鐘腳本見 [DEMO_SCRIPT](../operations/DEMO_SCRIPT.md)。目前 10/10 只涵蓋固定 Nav2
route legs；對外宣稱「三分鐘 hero demo 可重複」前，仍須固定同一 clean commit、模型、
場景與地點連跑 10 次完整 demo，至少 9 次在三分鐘內成功並保存時間與所有失敗。在此之前
只稱排練。這是銷售 demo gate，不等於安全認證。
