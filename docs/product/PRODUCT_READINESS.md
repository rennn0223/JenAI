...
JenAI v2.6.0 正式版是一個**受監督、具執行邊界的 ROS 2 高階決策與工作流代理**。它使用自然語言
或 Slash 指令理解任務、查詢 live ROS graph、選擇已註冊能力、經批准與可選 Twin Gate
驗證後呼叫 Nav2／ROS 2 API，最後以 odom、Nav2 result 與 audit 回報結果。

它不是底層控制器、未知空間 frontier explorer、經功能安全認證的產品，也尚未具備常駐的
`perceive → decide → rehearse → act → feedback` 自主迴圈。`/explore` 是已知儲存點位的
有界巡遊，不是 SLAM 探索。

目前 repository 是 public；v2.6.0 Release 公開提供 wheel、matching constraints、CycloneDX SBOM、`SHA256SUMS`，以及 build provenance 與 SBOM 的 Sigstore bundles。只有資產實際出現在 Release 且 checksum／attestation 驗證通過，才視為已發布與可驗證。

下表以 v2.6.0 tag 指向的 main@`a648576efa35b7f0ed8a376d34ef88ab8c1a5b18` 為固定發布 revision；[三版本 CI／build](https://github.com/rennn0223/JenAI/actions/runs/30592726527)、[Supply Chain](https://github.com/rennn0223/JenAI/actions/runs/30592726522) 與 [Release run](https://github.com/rennn0223/JenAI/actions/runs/30592758650) 均成功，[v2.6.0 Release](https://github.com/rennn0223/JenAI/releases/tag/v2.6.0) 已發布七個 allow-listed 資產。本機 fingerprint 證據保留為防漂移輔助；遠端 revision、tag、run
與實際下載資產才是發布事實來源。

v2.6.0 已於 2026-07-31 08:13（臺灣時間）發布 wheel、sdist、matching constraints、
CycloneDX SBOM、`SHA256SUMS` 與兩份 Sigstore bundles。發布完成不會把較早的 dirty
Isaac Sim／TUI 場次升格成 clean-revision HIL，也不會把模擬 STOP 證據外推為實體停止。
完整內容、驗證與限制見 `docs/releases/v2.6.0.md`、`docs/validation/ISAAC_HIL_ACCEPTANCE.md`
與 `docs/validation/TUI_LIVE_ACCEPTANCE_2026-07-26.md`。

## 六角色驗收矩陣

| ID | 角色 | 驗收條件 | 目前證據 | 狀態 | 關閉條件 |
|---|---|---|---|---|---|
| ENG-1 | 工程師 | 無 ROS 的單元／整合測試、lint、三版本 CI、wheel 冒煙全綠 | v2.6.0 的 Python 3.12／3.13／3.14、build、coverage gate、audit／SBOM 與 Release workflow 全綠；發布前本機 1,352 項測試、Ruff、strict mypy、網站 typecheck／build／5 項測試與 wheel lifecycle 均通過 | PASS | 後續每個 PR／main revision 持續維持三版本 CI 與 safety-chain gate |
| ENG-2 | 工程師 | ROS／Isaac 關鍵路徑可自動回歸，不只人工 TUI 實測 | v2.5.1 PR 前候選 HIL 完成 `map_left_down` 0.032 m／0.143 rad、Dock 0.036 m／0.142 rad、cancel acknowledgment、0.0031 m 停止漂移、final halt 與 bridge shutdown；真實 TUI 另驗 `/stop`、五點列舉、自然語言唯讀與末輪 Dock 0.046 m／0.090 rad，修正前失敗仍保留 | PASS | 每個候選 release 維持同 protocol 回歸；separated-domain Twin verdict 另由安全／部署 gate 驗收 |
| ENG-3 | 工程師 | 核心模組職責可維護 | TUI 批准生命週期、直接執行、指令分派、面板與位置命令皆維持獨立模組；Ruff 複雜度上限 10、strict mypy 與 1,352 項完整回歸全綠，行數不再作為品質替代指標 | PASS | 新功能維持 approval policy／execution／rendering 邊界與行為測試 |
| ENG-4 | 工程師 | 依賴與供應鏈可稽核 | v2.6.0 已發布 wheel、sdist、matching constraints、CycloneDX SBOM、`SHA256SUMS` 與兩份 Sigstore bundles；七個被列資產均由 Release workflow 完成 checksum 與資產集合驗證 | PASS | 每週持續掃描；每版維持 immutable tag/source 與 public attestation gate |
| PM-1 | PM | ICP 與主要任務明確 | 主要 ICP：已有 ROS 2／Nav2 的研究室與機器人開發團隊；主要任務：高階任務觸發與 ROS 開發輔助 | PASS | 新功能必須服務主要任務之一 |
| PM-2 | PM | v2 與 post-v2 承諾分開 | v2.6.0 正式版是 Workflow-first 的受監督代理；語意區域覆蓋不等於未知空間探索，Robot Runtime、TypeScript operator WebUI 與 NXDog motion 仍是後續工作 | PASS | README、論文、demo 不得把未驗證能力當成現有能力 |
| PM-3 | PM | 新手可從安裝走到第一個成功任務 | ONBOARDING、doctor 與 responsive TUI 已有；三位使用者曾在指導下試用，但未做純冷啟動計時 | PARTIAL | ≥5 位新手只看文件完成任務並保存時間／卡點 |
| PM-4 | PM | TUI 資訊層級、鍵盤流程與窄螢幕可用 | Claude Code 風格雙欄 welcome、平面 transcript／approval、composer 與 3 級 responsive 已實作；v2.5.1 發布前於 2026-07-26 實際驗收 doctor、批准卡、移動中急停、Dock 與狀態查詢；視覺外觀維持使用者核准版本 | PASS | 本版視覺封版；未來若改外觀，仍須先提供獨立樣本取得使用者批准 |
| BIZ-1 | 經營者 | 授權與發布可供外部採用 | Apache-2.0、public repository 與公開 v2.6.0 signed Release 已就緒；匿名使用者可下載並驗 checksum | PASS | 後續啟用適當 main／tag 保護並維持公開交付 |
...
| RES-3 | 教授 | 跨載具主張符合證據 | Vehicle Profile 與高階 API 支援介面可移植；物理泛化未驗證 | PARTIAL | 至少一個非 Ackermann 平台完成固定 PoC 任務集 |
...
| BUY-1 | 買家 | 能直接安裝、啟動、診斷與移除 | v2.6.0 public Release 可匿名下載；Release workflow 以 matching constraints 完成大小寫入口、version／help、無設定 doctor 與 uninstall。尚缺非維護者 fresh-machine 冷啟動 | PARTIAL | 由非維護者在 fresh machine 只照 README 完成下載、驗證、onboard、doctor 與移除 |
| BUY-2 | 買家 | 資安與部署邊界清楚 | `SECURITY`、`THREAT_MODEL`、`SUPPORT` 已明示 `/shell`、DDS、雲端資料外送與功能安全限制；v2.6.0 已發布逐版 CycloneDX SBOM／checksum 及 build／SBOM Sigstore bundles | PASS | 每個企業場域仍須另做 deployment threat review |
...
|---|---|---|---|
| 工程師 | `SATISFIED_WITH_EXTERNAL_GATES` | PR #121 三版本 CI／build／audit-SBOM、v2.4.0 release lifecycle，以及品質報告的完整 HIL／cancel evidence | separated-domain Twin verdict；每版 self-hosted workflow artifact |
...
| 經營者 | `SATISFIED_WITH_EXTERNAL_GATES` | `LICENSE`、`SUPPORT.md`、`docs/product/HANDOFF.md`、`docs/operations/ROLLBACK.md`、public v2.4.0 Release 與 published-release mutation guard | main／tag 保護仍待確認；第二維護者零介入演練 |
...
| 買家 | `SATISFIED_WITH_EXTERNAL_GATES` | `docs/QUICKSTART.md`、`docs/operations/ROLLBACK.md`、`docs/operations/SUPPORT_MATRIX.md`、`docs/operations/DATA_LIFECYCLE.md`、public v2.4.0 Release 與 clean wheel lifecycle | 非維護者 fresh-machine 驗收；本機 data harden 仍等待使用者 opt-in |
...
歷史發布證據仍保留；v2.4.0 已於 public repository 發布 signed assets。本機資料尚未獲准
