...
JenAI v2.2.0 候選是一個**受監督、具執行邊界的 ROS2 高階決策與工作流代理**。它使用自然語言
...
|---|---|---|---|---|---|
| ENG-1 | 工程師 | 無 ROS 的單元／整合測試、lint、三版本 CI、wheel 冒煙全綠 | v2.2.0 候選本機完整 pytest、Ruff、mypy strict、build／隔離 wheel smoke 全綠；遠端三版本 CI、build 與 audit/SBOM 只在本版 PR 實際通過後成立。v2.1.0 Release 是上一版獨立證據 | PASS | 後續每個 PR／main revision 持續維持三版本 CI 與 safety-chain gate |
...
| PM-2 | PM | v2 與 post-v2 承諾分開 | v2.2.0 候選仍是受監督工作流代理；唯讀快速路徑不等於 M6 常駐決策迴圈，後者仍移至 post-v2（候選 v3） | PASS | README、論文、demo 不得把 M6 當現有能力 |
