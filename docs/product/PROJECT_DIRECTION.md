...
(2) **互補式安全邊界**(各控制處理不同危害，不假設彼此獨立);(3) **Twin-Gated Execution**——指令先在 Isaac Sim
數位孿生跑一遍、過閘門才碰實體。近年已有 LLM 機器人之執行前驗證與
LLM×數位孿生相鄰工作；JenAI 的差異化主張限於把 ROS live-graph 發現、人工權限、
逐任務預演、結果回授與稽核整合成同一套可重播工作流，不主張「別人都沒有」或首創。
...
要不要人批准」。目前 DGX Spark + qwen3.6:35b 的 E4 固定快照中位數為 68.8 秒，
所以只適合高階任務；需要穩定操作節奏時使用確定性 Slash 能力。本地部署解的是
資料與離線可用性，不代表模型回應只有一兩秒。`/provider` 可切換本地端點，斷網時
已載入的本地模型仍可使用。
...
2. **方法貢獻**:以觀察、決策、能力、權限與執行驗證五類邊界包絡高階 Agent；
   Isaac Sim 數位分身預演是執行驗證的一種機制，與 HITL 針對不同危害互補，但未證明
   統計獨立或正交，也不是整套系統的名稱。
3. **實證貢獻**:E1 高階決策、E2 固定目標政策的描述性配對重分析（A／B derived、
   C observed）、E3 domain 42 mock ROS fixture 的自然語言閉環工具使用、E4 本地模型延遲，以及
   B4 可重建固定 subset 的 Isaac Sim 導航任務紀錄。B4 不支持精確 20 h 暴露或零事件。
   先期 Ackermann 同指令
...
```
   ...
   provider 抽象:NVIDIA NIM (雲) ⇄ Ollama (DGX Spark 地端),斷網可用
        │
        ▼
                【閘門層】Digital Twin — 時間尺度:秒~分(選配,可關)
...
| M3 | **Isaac Sim twin 場景 + Twin Gate pipeline** | PM | ✅ **sim v1.0** | G1–G5、pass/block/refer、可設定獨立 ROS_DOMAIN_ID、doctor `twin` 與導航單一出口已實作；歷史單一 Isaac Sim/domain 0 場次不構成通訊隔離驗證。E2 固定目標比較僅 C observed、A／B derived；B4 只支持固定 subset 的 102 份 reports／407-of-408 waypoint succeeded，不能主張精確 20 h 或零事件，H9 另列。實體場景同步與 Sim-to-Real 仍是未來工作 |
...
**一句話收斂**:JenAI 已完成「載具介面無關的受監督任務層 Agent + 互補式執行邊界」之
Ackermann 模擬功能驗證；這不是功能安全認證或零事件證據。M1–M4 已落地，M5 尚缺純冷啟動量測。要操作四足，缺的不是讓
...
推論:v1 奠定受監督高階決策、ROS2 開發輔助與不可繞過的執行邊界；v2.0 收緊
批准／取消安全語意並補齊產品化生命週期。M6 常駐決策迴圈移至 post-v2（候選 v3）
研究方向，且目前未實作。`/drive`、`/ros pub`、odom 直驅降級為工程工具；
「感知深化」改向餵決策快照(語意),不餵輪子(幾何)。
負面結果(depth-only 避障不可行)寫入論文第五章失效分析——委託原生
堆疊是量測後的設計決策,不是省事。

### 驗證策略更新(2026-07)—— Isaac Sim 為主要驗證平台

受監督 v1 架構(監督/決策/閘門/技能/反射/橋接)的驗收證據**以 Isaac Sim
孿生場景為主**:接口確認、地點建置、任務實測、固定任務紀錄與政策比較都在 twin
