...
### 驗收條件
- [ ] 每個 fail 都有具體可執行的 fix_suggestion
- [ ] provider 檢查能區分「base URL 不可達」與「model 不存在」
- [ ] ROS2 檢查能區分「ros2 指令不存在」與「環境未 source」
...
### 驗收條件
- [ ] 敏感工具一律進入 `awaiting_approval`，不可繞過
- [ ] 批准後從原狀態繼續，而非重新開始
- [ ] 拒絕後有可讀的後續處理，不允許靜默跳過
...
### 驗收條件
- [ ] 能串起 topic-info 與 interface show
- [ ] example_payload 格式合法，可直接用於 `/ros pub`
...
### 驗收條件
- [ ] 未批准不得執行
- [ ] stdout / stderr 有結構化顯示
- [ ] 高風險命令額外加強警示（p2 level）
