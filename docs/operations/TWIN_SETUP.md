...
倒置佈局的啟用方式:config 設 `deployment_mode = "physical"`、`[twin] domain_id = 0`、
`[vehicle] domain_id = 20`、`enabled = true`。純模擬驗證保留
`deployment_mode = "simulation"` 且 JenAI 使用 `ROS_DOMAIN_ID=0`；實體部署才以
...
都應走這條。只有在兩個 domain 實際不同且 §2 隔離檢查通過時，才可說該次 DDS
probe 未觀察到跨域傳遞；這仍不是對設定錯誤、外部 bridge 或物理安全的絕對保證。
...
2. **起孿生側 Nav2**(另一個 shell,同樣先設 domain):
   ...
   若日後接真車,則兩側都用真車的地圖)。
3. **驗收隔離**（以下以 twin=42、vehicle=0 為例；倒置佈局交換數字）:
   ```bash
   ROS_DOMAIN_ID=42 ros2 action list   # 正向檢查：孿生 graph 應看到 /navigate_to_pose
   ROS_DOMAIN_ID=42 ros2 topic pub -r 2 /jenai_isolation_probe std_msgs/msg/Bool "{data: true}"
   # 另一個 shell：預期 timeout，不能收到 twin probe
   timeout 3 env ROS_DOMAIN_ID=0 ros2 topic echo /jenai_isolation_probe --once
   jenai doctor                        # twin_isolation 也必須 pass
   ```
   記錄正向 graph、反向 probe 與 doctor 輸出；只看到空的載具清單不足以證明隔離。
   測完以 Ctrl-C 停掉 probe publisher。
...
沒有這個 topic 時 G1 會標記 `skipped`、整體預設為 `refer`，不會把「沒有碰撞證據」當成通過。只有研究場景明確設定 `require_collision_evidence = false` 才允許其餘判準決定結果，報告仍保留 G1 skipped。**一鍵重建**:場景重載後感測器不見時,Script Editor 執行 `~/isaac_scenes/add_contact_sensor.py` 自動重建感測器與發布 graph(可重複執行)。
...
```toml
deployment_mode = "physical" # 實體車可能上線時必填；同 domain 會 hard fail
...
require_collision_evidence = true      # 預設；缺 contact evidence 時 verdict=refer
...
`goal_tolerance_m` 必須依載具運動學、local planner 與場景校準。2026-07-17 的 Isaac Sim
Ackermann 驗收觀察到 Nav2 回報成功時終點偏差約 0.75–0.77 m，因此該場景設定使用
`0.8` m；這是模擬驗收門檻，不是可直接套用到實車的通用值。論文或報告應同時揭露
觀察偏差、設定值與校準環境，避免把放寬容許值寫成定位精度提升。

這個 `[twin].goal_tolerance_m` 是 G4「孿生預演是否可接受」的判準，不是載具到點後
JenAI 對外宣告成功的精度。後者由 `[vehicle].arrival_position_tolerance_m` 與
`arrival_yaw_tolerance_rad` 控制；目前 Carter 模擬精準 profile 為 0.05 m／0.15 rad。
兩套門檻用途不同，不得把歷史 G4 的 0.8 m 當成現在的 Nav2 到點標準。

> **純模擬例外。** 2026-07-17 的互動驗收由使用者刻意令 `twin.domain_id=0`，使預演與
> 執行作用於同一部 Isaac Sim 車。這可驗證 G1–G5、批准與回授鏈，但**沒有通訊隔離**，
> `doctor` 在 simulation 模式會明確警告「沒有隔離」；physical 模式則 hard fail。只要實體載具可能上線，就必須恢復不同
> ROS domain、橋接白名單或等效隔離；純模擬開發期間則讓實體車關機。

改完跑 `jenai doctor`:`twin` 區段 twin_graph / twin_nav2 / twin_contact_sensor
三項 pass 即為 **B5 場景建置完成**,之後每個導航目標都會先進入執行驗證。
...
4. `/route 從A到B` 實測:即時剩餘距離、Esc 真取消。這一步通了就能開始
   B3(TEST.md 解鎖)與 B4(有 run ID／duration／incident 欄的模擬導航任務紀錄)累積。
...
- 兩個 Isaac 實例太重:孿生側只在跑 Gate 實驗(B5 固定目標政策比較)時才開;平日跑
