...
- **Release**(`.github/workflows/release.yml`):推 `vX.Y.Z` tag 觸發 —— 驗 tag 與 pyproject 版本一致、重跑 lint+測試、`uv build` + wheel 冒煙測試,建**草稿** release(自動 notes + wheel/sdist 附件);發佈走人工閘:`gh release edit vX.Y.Z --notes-file docs/releases/vX.Y.Z.md --draft=false`(手寫 notes 自 v0.23 版本化在 `docs/releases/`,隨 PR review)。tag 已有 release 時只補上傳附件
