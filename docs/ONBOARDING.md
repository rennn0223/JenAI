...
沒有真車時,以上流程對 Isaac Sim 同樣成立:場景裡的車發 `/odom`、`/scan`(需在 USD 裡掛 LiDAR)、訂 `/ackermann_cmd`(裝 `cmdvel_to_ackermann` 轉換後即 `/cmd_vel`)。相機 topic 通常是 `/rgb`——把 `[vehicle] camera_topic` 改過去,`/vision camera`、`/patrol … photo` 就直接可用。孿生與實車同時在線時,請用不同 `ROS_DOMAIN_ID` 隔離。
