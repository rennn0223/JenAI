...
| # | 危害 | 成因 | 防護 | 驗證證據 | 殘餘風險 |
|---|---|---|---|---|---|
| H1 | 撞到人/障礙物 | 導航中環境變化;規劃錯誤 | **R**:避障是 Nav2/controller 的事(毫秒級,LLM 不在迴路);odom 避障 depth 逾時 fail-closed;**G**:goal 先在孿生預演,G1 碰撞即 block | test_architecture、test_avoidance(freshness/走廊)、test_twin_gate；B4 subset 可重建 102 份 sim 任務報告、407/408 waypoint succeeded，但 report 無 incident 欄且無獨立觀察者，不能作「0 安全事件」證據；HIL-FS2-20260719 在固定模擬環境完成兩條 route 且 0 recoveries，Hero 補充 10/10 route legs（0–4 recoveries），但兩者均未設獨立碰撞觀察欄，不能作「0 安全事件」證據；實車避障未量測 | 感測器盲區、twin 與實景偏差、事件漏記 —— 靠限速、監督與後續獨立觀測減輕 |
...
| H4 | 通訊失效後持續移動 | TUI/daemon 崩潰、網路斷、行程被 kill、Nav2 terminal future 損壞 | **R**:bridge watchdog —— client 斷線/卡死 >6s 自主停車,每 2s 重發 halt;watchdog 武裝失敗 = 啟動失敗；Nav2 terminal 無法驗證時先保留 active/watchdog、執行 emergency halt，再清狀態；odom freshness 超時立即歸零 | test_bridge_client(武裝失敗擋啟動、stderr 診斷)、test_bridge_state(terminal 驗證)、test_drive_control(stale/future/non-finite odom)、v0.8 實測(watchdog 斷線停車) | DDS 本身斷連時 halt 到不了 —— 硬體 estop 是最後防線(🅿 選配:實車硬體 estop 配置,P 項;sim 驗證以 watchdog 斷線停車 + /stop 實測覆蓋) |
| H5 | AI 聽錯/亂決策(意圖層錯誤) | LLM 誤解自然語言;VLM 誤判場景 | **H**:自動模式只允許可記憶的 bounded non-host P0/P1 操作略過批准卡；`HOST_COMMAND` 或 P2 每次均須明確批准、不可 session remember；P2、host command 與 robot control 預設 No；daemon 自主動作仍需 `auto_approve`+nav2 明式授權；感知規則走同一套 gating 無捷徑；twin refer 在自主路徑視為 block | test_daemon(twin refer → NOT moved、confidence 亂填不觸發)、批准政策與工具分級驗收(TEST.md) | 人仍可能批准語意錯誤的動作；G 只檢查已建模執行後果，不能保證補救意圖錯誤 |
| H6 | 緊急停止失效 | 停止路徑本身出 bug;停止被排隊；bridge 請求／回覆遺失或型別畸形導致狀態不明 | **R+設計**:`/stop` 免批准、Esc 取消不了停止本身、halt publisher 預建；Esc、timeout 與傳送結果不明皆執行 cancel-all + zero pulses；`/stop` 另採 halt→cancel/reap producer→final halt；client 與 sidecar 皆拒絕 truthy 字串、NaN/Inf、無效範圍，halt/cancel 回執須為精確 bool 且 `halted=true`；direct-drive 最終歸零失敗改報 `halt_failed` 並保持 watchdog fault latch；HIL 任一 motion 失敗即中止後續動作，`final_halt` 成敗強制落盤；WebUI STOP 免 token、不等待 body且撤銷舊確認 | test_safety_order、test_nav_live(ambiguous dispatch/timeout/Esc)、test_bridge_client(request/response malformed)、test_bridge_protocol(sidecar fail-before-dispatch)、test_bridge_state(watchdog validation)、test_isaac_hil(fail-fast/final halt)、test_drive_control(halt terminal honesty)+安全鏈覆蓋 CI；clean HIL-FS2-20260719 記錄 local task canceled、Nav2 cancellation acknowledged=true、zero-velocity software halt 與停止後漂移 0.0000 m（sim） | 多重防護後仍是軟體；若 bridge 與 DDS 同時不可達，軟體無法證明車已停——硬體 estop 為最後防線 |
...
`doctor` 必須依實驗依賴判定：E2/B4 要求 ROS/Nav/map/pose/Twin readiness；Isaac HIL 另要求合法起點及 10 筆 `/scan` 的樣本數、幾何、時間、frame、timestamp、range 與 coverage 門檻；E3 要求
