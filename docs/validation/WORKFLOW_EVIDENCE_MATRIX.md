...
| 自然語言唯讀狀態可走 deterministic fast path 且不移動 | `JenAITuiApp._handle_plain_language`、`_show_state_inspection` | `test_plain_language_routes_by_mode`、`test_state_request_with_non_actuation_constraint_remains_read_only` | 2026-07-26 唯讀自然語言場次 | `ENFORCED` | 只涵蓋已註冊反射意圖，不代表所有自然語言皆具確定性 |
...
| 端點恢復只能在取消獲確認後有限重送 | `jenai.tools.nav_live.navigate_live` 的 endpoint recovery contract | `test_navigate_live_retries_one_confirmed_near_endpoint_stall`、`test_navigate_live_does_not_retry_without_confirmed_nav2_cancellation`、`test_navigate_live_bounds_a_failed_endpoint_recovery_retry` | 2026-07-26 pre-PR `map_left_down` 一次有界恢復 | `ENFORCED` | 實體底盤尚未驗證 |
