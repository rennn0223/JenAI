...
mkdir -p artifacts/experiments/e1
uv run JenAI eval scenarios.e1.toml --json > artifacts/experiments/e1/e1-$(date +%Y%m%d).json   # 留原始數據
```
...
OUT=artifacts/experiments/e2/e2-$(date +%Y%m%d)-paired
...
OUT=artifacts/experiments/e2/e2-$(date +%Y%m%d)-paired-formal
...
uv run python scripts/e3_agent_bench.py --out artifacts/experiments/e3/e3-agent-boundary-$(date +%Y%m%d).jsonl
```
...
```bash
uv run python scripts/e4_bench.py --n 100 --out artifacts/experiments/e4/e4-local-$(date +%Y%m%d).jsonl
uv run python scripts/e4_bench.py --n 100 --provider nvidia-cloud \
    --model meta/llama-3.3-70b-instruct --out artifacts/experiments/e4/e4-cloud-$(date +%Y%m%d).jsonl
```
...
## HIL-FS｜Isaac FullScan route／cancel／stop

這是人工啟動、會讓 Isaac 模擬車移動的正式 gate；一般 CI 不執行。先確認 RTX LiDAR
Helper 的 `Publish Full Scan=True`，10 Hz full cloud 對應 converter `scan_time=0.1`。
只看 `/scan` 頻率不足以證明掃描品質。
...
uv run python scripts/isaac_hil_acceptance.py \
  ...
  --output artifacts/isaac-hil-preflight-<date>.json

# 看過 preflight 與模擬畫面、確認路徑安全後才執行：
uv run python scripts/isaac_hil_acceptance.py \
  --execute \
...
- 輸出:`artifacts/experiments/soak/soak-<ts>/report.md`(要 **PASS**)+ `rss.csv` + `daemon.log`
- ✅ 已完成:`artifacts/experiments/soak/soak-20260715-012527/` PASS(RSS +1.2%,限 20%)
