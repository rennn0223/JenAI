...
> 對應版本:v2.6.0(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| Isaac navigation differential（人工啟動） | 先完成 observation-only Code Review，再依 `docs/validation/ISAAC_NAV_DIFFERENTIAL.md` 跑一組不納入統計的 R1/R2 pilot | [ADR 0007](../adr/0007-simulation-differential-control-arm.md) 只允許 R1 在 simulation acceptance harness 中以 harness-owned bridge 建立 Nav2 control；R2 仍走 Navigation Gateway。只有 schema-v1 envelope、evidence derivation v2、clean simulation／Python／RMW identity、不可變 target binding、明確 motion confirmation、active Stage SHA、measurement contract、append-only pose journal 與 T0/T1/final-window/cleanup 全 PASS 的 `captured` artifact 才能比較。此例外不是 Capability、產品 motion path 或實體證據；沒有可信 `T_map_world` 時只能回報 `MAP_POSE_DIFFERENCE`，不得宣稱實際終點差異；不得改 Nav2／AMCL／retry／容差。 |
| Isaac Motion Safety Gate（不移動） | `uv run python scripts/isaac_motion_readiness.py validate --artifact <artifact>` | 依 ADR 0008 從 raw Evidence 重新推導 orientation-specific swept footprint、USD/Nav2 footprint attestation、collision timeline、七項 clearance budget 與 PASS/BLOCK。缺少 collision source time、controller tracking 或 stopping bound 必須 BLOCK；此 Gate 不送 goal／cmd_vel，也不把 simulation 結果外推成實體安全。 |
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 2.6.0`(版本來自 package metadata,隨 release 走) |
...
| 狀態 | 指令 | 測法 | 期望輸出 |
|---|---|---|---|
| ✅ | `/route 從應科大樓到機械系館` | 建點 + Nav2 | **兩端已知 → 先去 A 再去 B**(兩段導航);只認得目的地 → 從當前位置去。即時剩餘距離、Esc 真取消。Nav2 `SUCCEEDED` 後仍須通過 `[vehicle]` terminal-pose 位置／yaw 二次核對，超限即 halt+failed。2026-07-19 clean HIL：cancel acknowledged=true、停止漂移 0.0000 m；2026-07-24 dirty 工程樣本同步 DWB／goal checker xy=0.05 m 後 terminal feedback 0.04 m（非正式主證據） |
| ✅ | `/drive 前進兩秒` | 批准後 | 自然語言 → 速度指令定時發布到 `vehicle.cmd_vel_topic`,結束自動停(⚠️ 實體會動) |
| ⛔ | legacy 局部避障（`[avoidance] enabled=true`, `route_adapter=odom`） | 非產品驗收 | 僅保留 bridge bring-up 與歷史回歸；NavigationGateway 拒絕高階任務使用 `route_adapter=odom`。產品避障由載具既有 Nav2 controller 負責 |
...
| 狀態 | 指令 | 測法 | 期望輸出 |
|---|---|---|---|
| ✅ | `/shell ls -la` | 每次明確選 Yes 後 | 任意 shell 一律 P2、預選 No、不可 remember；命令輸出進時間軸 |
| ✅ | `!ls -la` | 每次明確選 Yes 後 | 同 `/shell` |

### Twin Gate(M3)

| 狀態 | 項目 | 期望輸出 |
|---|---|---|
| 🧪 | Gate pipeline(G1–G5 判準、pass/block/refer、refer→daemon 視為 block) | Twin Gate 單元測試涵蓋；G1 evidence 預設必需、缺 contact topic 時 `refer`；`[twin]` 預設關閉 = 零行為變化 |
...
| 狀態 | 測法 | 期望輸出 |
|---|---|---|
| 🧪✅ | 任何需批准指令(`/ros pub`、`/drive`、`/route`、`/mission`、`/patrol`、`/dock`、`/shell`、`/run` 內 side-effect) | Claude Code 風格風險感知編號卡；有界非 host P0/P1 可選 session remember，HOST_COMMAND/P2 僅一次性 Yes/No；P2、host command 與 robot control 預選 No，auto/remember 皆不可繞過；`/dock` 與 `/route` 的 remember **互不洩漏** |
