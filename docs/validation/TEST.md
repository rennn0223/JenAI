...
> 對應版本:v2.2.0(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 自動化測試(全) | `env -u PYTHONPATH uv run pytest` | v2.2.0 工作樹以 Python 3.12／3.13／3.14 驗證；branch coverage 基準 77%，安全鏈 94%。全部 `src/jenai` production code 採 mypy strict。精確測試數與通過狀態以當次命令／CI summary 為準；遠端 CI／release 證據仍只在對應 GitHub Actions run 實際通過後成立 |
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 2.2.0`(版本來自 package metadata,隨 release 走) |
