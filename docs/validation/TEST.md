...
> 對應版本:v2.4.0(快照隨 release 更新)。所有可測項目(CLI / Slash / 對話)與期望輸出的總表,
...
| 項目 | 指令 | 期望輸出 |
|---|---|---|
| 自動化測試(全) | `env -u PYTHONPATH uv run pytest` | v2.4.0 候選版在本機 1,066 項測試通過；branch coverage 79%，安全鏈 94%。全部 `src/jenai` production code 採 mypy strict；遠端 Python 3.12／3.13／3.14、build、audit/SBOM 與 release 證據須等本版 PR／Release workflow 實際通過，不能沿用 v2.3.0 證據 |
...
| 狀態 | 命令 | 期望輸出 |
|---|---|---|
| ✅ | `JenAI version` | `JenAI 2.4.0`(版本來自 package metadata,隨 release 走) |
