...
- preflight 會抽樣 10 筆 `/scan`；全空樣本比例必須 ≤20%、aggregate finite-bin coverage 必須 ≥25%，NaN、`-inf`、空 ranges 或 malformed bins 一律 fail。topic 有頻率本身不算通過。
...
2026-07-19 已在 clean revision `fb5645620c787bd54fc8368fe402366371561b1e` 完成正式 live run。
本機 artifact `artifacts/isaac-hil-live-fullscan-guard-fb56456-20260719.json`（SHA-256
`51b3a7451273fd3e361d8f40c251ebdf1350d7c476c0fe913e2c7509af129d00`）記錄：scan
10/10、0 blank、53.7569% finite；`map_left_down` 與 Dock route 均成功且 0 recoveries；
cancel 傳遞成功、停止後漂移 0.0000 m。整體為 `pass_with_skips`，因 target 與 Twin
都在 domain 0，`twin_isolation` 明確為 `skip`。詳見
[TUI_LIVE_ACCEPTANCE_2026-07-19.md](TUI_LIVE_ACCEPTANCE_2026-07-19.md)。

這份本機 artifact 不進 Git 且不取代歷史失敗樣本；也不支持實體安全、sim-to-real、
