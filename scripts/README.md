...
| 檔案 | 用途 |
|---|---|
| `soak.py` | 24h 穩定性測試(V1_GATE A6):啟動 daemon、樹狀 RSS 採樣、warmup 校正、PASS/WARN 判定。`python3 scripts/soak.py --rules rules.example.toml`;短跑自檢:`--minutes 5 --interval 5 --warmup 60`。輸出 `artifacts/experiments/soak/soak-*/{rss.csv, report.md, daemon.log}` |
...
| `e4_bench.py` | E4 決策延遲量測:固定快照重複 decide(),量中位/P95。`uv run python scripts/e4_bench.py --n 100 --out artifacts/experiments/e4/e4-local.jsonl` |
