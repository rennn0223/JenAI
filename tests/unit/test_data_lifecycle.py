...
def test_export_excludes_unmanaged_json_below_reports(tmp_path: Path) -> None:
    ...
    (paths.reports / "area-patrol-owned.json").write_text("{}", encoding="utf-8")
    (paths.reports / "evidence-owned.png").write_bytes(b"image")
    ...
    assert "reports/area-patrol-owned.json" in names
    assert "reports/evidence-owned.png" in names
...
def test_prune_removes_old_files_and_only_old_trace_rows(tmp_path: Path) -> None:
    ...
    old_evidence = paths.reports / "evidence-old.png"
    ...
    nested.parent.mkdir()
    for path in (old_session, old_report, old_evidence, recent_report, old_task_receipt):
    ...
    os.utime(old_evidence, (old_timestamp, old_timestamp))
