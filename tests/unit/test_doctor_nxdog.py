...
def test_doctor_does_not_call_untimestamped_map_observation_fresh() -> None:
    items = check_nxdog(observer=_Observer(_snapshot(current_map=None)))  # type: ignore[arg-type]
    item = next(item for item in items if item.check_name == "current_map")

    assert item.message == "NXDog reports no current map group name."


def test_doctor_warns_when_current_map_and_odom_map_disagree() -> None:
    items = check_nxdog(
        observer=_Observer(_snapshot(current_map="lab", pose_map="warehouse"))  # type: ignore[arg-type]
    )
    item = next(item for item in items if item.check_name == "map_consistency")

    assert item.status == "warn"
    assert item.message == "NXDog map observations are inconsistent and untimestamped."


def test_doctor_omits_map_consistency_warning_when_map_names_agree() -> None:
    items = check_nxdog(observer=_Observer(_snapshot()))  # type: ignore[arg-type]

    assert all(item.check_name != "map_consistency" for item in items)
...
def test_doctor_fails_the_specific_endpoint_without_erasing_other_checks() -> None:
    ...
    assert "[invalid_payload]" in by_name["velocity_observation"].message
    assert by_name["velocity_observation"].fix_suggestion
    assert "network path" not in by_name["velocity_observation"].fix_suggestion
    assert by_name["pose_observation"].status == "pass"


def test_doctor_preserves_internal_adapter_failure_category() -> None:
    failure = NXDogEndpointFailure(
        endpoint="/velocity",
        kind=NXDogFailureKind.INTERNAL_ADAPTER_ERROR,
        message="/velocity adapter failed: RuntimeError",
    )
    observation = _snapshot(failures=(failure,)).model_copy(update={"velocity": None})

    items = check_nxdog(observer=_Observer(observation))  # type: ignore[arg-type]
    item = next(item for item in items if item.check_name == "velocity_observation")

    assert "[internal_adapter_error]" in item.message
    assert item.fix_suggestion
    assert "network path" not in item.fix_suggestion
...
def test_doctor_reports_malformed_ipv6_as_configuration_failure_instead_of_crashing() -> None:
    items = check_nxdog(environ={NXDOG_API_URL_ENV: "http://[::1"})
