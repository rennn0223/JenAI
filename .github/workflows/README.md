...
| `release.yml` | 推 `v*` tag；或人工 dispatch | 驗 tag、trigger SHA、remote tag 與 pyproject 版本一致 → lint／完整測試／安全 coverage／dependency audit → reproducible build → sdist 敏感檔掃描 → matching constraints、CycloneDX SBOM、SHA256SUMS；public repo 另強制 provenance 與 SBOM attestations → fresh wheel lifecycle → draft／人工發布 |
