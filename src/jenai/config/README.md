...
| 檔案 | 職責 |
|---|---|
| `models.py` | `AppConfig` + provider profiles + **`VehicleProfile`(`[vehicle]`:cmd_vel topic、硬限速、相機 topic)** + `TwinProfile`(`[twin]`:孿生 domain、G1–G5 閾值、禁區)+ `MapDatum`(`[map_datum]`:map 原點經緯度與軸向,`/loc add gps` 的換算基準)+ `AvoidanceProfile`(`[avoidance]`:depth camera follow-the-gap 反應式避障,給 odom 直驅用)。**全 repo 唯一允許出現載具差異的地方** |
