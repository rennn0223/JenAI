...
| 檔案 | 職責 |
|---|---|
| `app.py` | App 殼:輸入分發(裸語言依權限模式路由 / `!` shell / slash)、spinner、批准卡流程(自動模式明示自動批准)、mission/patrol/explore 執行與日報存檔 |
| `catalog.py` | 穩定的 `SLASH_COMMANDS` palette 資料源、standalone greeting 判斷與現行 CSS；從 app 搬出但不改任何視覺 token |
