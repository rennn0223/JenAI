...
driving, navigation (with the live rclpy bridge), camera vision and
perception. Saved-location persistence is inherited from LocationCommandsMixin;
approval plumbing and app state come from the host class.
"""
...
class RobotCommandsMixin(LocationCommandsMixin):
    ...
    async def _show_dock(self, _: str = "") -> None:
        ...
        approval = ApprovalRequest(
            ...
            summary=(
                f"The robot will navigate to '{dock.name}' ({dock.pose.x:.2f}, {dock.pose.y:.2f})."
            ),
