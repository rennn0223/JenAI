...
| 檔案 | 職責 |
|---|---|
| `server.py` | 端點:`/api/status` `/api/command` `/api/confirm` `/api/map` `/api/stop`;**token 認證**(Bearer/cookie/`?token=`,401 絕不 Set-Cookie);`_PendingConfirms`(動作 server 端一次性持有,瀏覽器改不了);PoseCache(退避重試);**`/api/stop` 是唯一免認證端點 —— 停車永遠安全** |
