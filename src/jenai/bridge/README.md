...
| 檔案 | 職責 |
|---|---|
| `ros_bridge.py` | **系統 Python** 下的 rclpy 節點:pose、nav_send/feedback/cancel、**drive_to_pose(無 Nav2 的 odom→cmd_vel 閉環直驅,開闊地用)**、**halt(急停)**、**watchdog(client 斷線 >6s 自主停車)**、capture_frame、watch。**此檔絕不 import jenai**(跑在 venv 外);測試用 `tests/unit/fake_bridge.py` 講同一套協定 |
