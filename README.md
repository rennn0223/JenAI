...
>
> ✅ 完整 Agent 架構（基於 openai-agents SDK）：**多-agent handoffs**（Supervisor + ROS/Motion/Navigation/Perception 專職 agent）、**跨 session 記憶**（`Session`）、**安全 guardrails** + 確定性速度夾限、**閉環感知**（`/ros state`）、**Nav2 導航**（誠實回報）、**確定性任務執行器**（`/mission`）、**本地 tracing**。詳見 [`docs/FEATURES.md`](docs/FEATURES.md)。
>
